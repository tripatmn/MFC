...
def make_timeline(indices: Iterable[int], params: dict[str, Any], duplicates: Iterable[int] = ()) -> Timeline:
    ...
    warnings: list[str] = []
    if adaptive and _number(params.get("t_save")) is not None:
        scale = float(params["t_save"])
        times = tuple(index * scale for index in saved)
        steps = tuple(None for _ in saved)
        basis = "inferred: saved_index * t_save (adaptive output index is not simulation step)"
    elif _number(params.get("dt")) is not None:
        ...
        steps = tuple(None for _ in saved)
        basis = "unavailable"
        warnings.append("physical time cannot be inferred from t_save or dt")
