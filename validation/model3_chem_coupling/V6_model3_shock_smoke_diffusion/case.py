#!/usr/bin/env python3
"""V6: small periodic Model-3 shock/HLLC/species smoke test."""
...
parser = argparse.ArgumentParser(prog="V6_model3_shock_smoke")
...
comp = f"{args.fuel_species}:0.005,O2:0.209,N2:0.786"

left = load_solution(args.ctfile, args.ctphase)
right = load_solution(args.ctfile, args.ctphase)

left.TPX = 900.0, 5.0 * 101325.0, comp
right.TPX = 300.0, 1.0 * 101325.0, comp

fuel_species_id = species_index(left, args.fuel_species) + 1

left_state = split_model3_gas_state(left, args.fuel_species, alpha_liquid=0.02)
right_state = split_model3_gas_state(right, args.fuel_species, alpha_liquid=0.02)
...
case = {
    ...
    # Periodic gives a globally conservative shock-tube smoke test.
    "bc_x%beg": -1,
    "bc_x%end": -1,
...
ids = selected_species_ids(left, [args.fuel_species, "O2", "N2", "CO2", "H2O"])
set_species(case, left, [1], ids)
set_species(case, right, [2], ids)
