...
parser.add_argument("--haehn-xe", action="store_true", help="Use the compact Haehn-style Xe shock-bubble ROI.")
parser.add_argument("--d0", type=float, default=0.04, help="Bubble diameter for ROI presets [m].")
parser.add_argument("--cells-per-d0", type=int, default=None, help="Resolution in cells per bubble diameter for ROI presets.")
parser.add_argument("--x-minus-d0", type=float, default=2.0, help="Domain extent upstream of the bubble center in D0.")
parser.add_argument("--x-plus-d0", type=float, default=7.0, help="Domain extent downstream of the bubble center in D0.")
parser.add_argument("--y-half-d0", type=float, default=3.2, help="Domain half-height around the bubble center in D0.")
parser.add_argument("--shock-offset-d0", type=float, default=1.0, help="Initial incident shock offset upstream of bubble center in D0.")
...
is_3d = args.ndim == 3
if args.haehn_xe and is_3d:
    raise ValueError("--haehn-xe is currently a 2D compact-ROI preset.")

if args.haehn_xe:
    D0 = args.d0
    cells_per_d0 = args.cells_per_d0 or 30  # coarse local AQSS smoke test
    Lx = (args.x_minus_d0 + args.x_plus_d0) * D0
    Ly = 2.0 * args.y_half_d0 * D0
    Lz = Ly
    Nx = int(round((args.x_minus_d0 + args.x_plus_d0) * cells_per_d0))
    Ny = int(round(2.0 * args.y_half_d0 * cells_per_d0))
    Nz = 0
else:
    Ly = 0.03
    Lz = Ly
...
if args.haehn_xe:
    R = 0.5 * args.d0
    x_bub = args.x_minus_d0 * args.d0
    x_shock = x_bub - args.shock_offset_d0 * args.d0
else:
...
case = {
    ...
    "parallel_io": "F",
    # Local checkout compatibility: these enums are strings upstream, integers here.
    "model_eqns": 2,
    "num_fluids": 1,
    "num_patches": 3,
    "mpp_lim": "F",
