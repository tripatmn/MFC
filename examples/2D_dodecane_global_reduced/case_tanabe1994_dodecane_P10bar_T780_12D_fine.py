#!/usr/bin/env python3
"""Tanabe-1994-inspired 12D fine validation-candidate n-dodecane/SK54 ignition case.

Fresh standalone two-patch setup:
  1. domain-filling hot air patch with trace liquid/vapor components
  2. centered circular liquid n-dodecane droplet patch altering patch 1

No shock, no imposed convection, no body force, no restart/continue logic.
The case keeps the established three broad fluids used by the current MFC
dodecane evaporation/combustion plumbing: liquid dodecane, dodecane vapor, air.
"""
...
parser = argparse.ArgumentParser(prog="case_tanabe1994_dodecane_P10bar_T780_12D_fine")
parser.add_argument("--mfc", type=json.loads, default="{}", metavar="DICT")
parser.parse_args()


# Tanabe-style high-pressure/high-temperature edge condition.
D0 = 0.7e-3
r_drop = 0.5*D0
domain = 12.0*D0
nx = 256
...
# Ideal-gas density proxies at 1.0 MPa / 780 K.
rho_v = p0/((gamma_v - 1.0)*cv_v*T_air)
rho_air = p0/((gamma_air - 1.0)*cv_air*T_air)
...
case = {
    ...
    "evap_species_alpha_min": 1.0e-4,
    "evap_species_mass_min": 1.0e-6,
...
# Audit command block:
# python3 examples/2D_dodecane_global_reduced/case_tanabe1994_dodecane_P10bar_T780_12D_fine.py > /tmp/tanabe1994_12D_fine.json
# python3 - <<'PY'
# import json
# d = json.load(open('/tmp/tanabe1994_12D_fine.json'))
# for k in sorted(d):
