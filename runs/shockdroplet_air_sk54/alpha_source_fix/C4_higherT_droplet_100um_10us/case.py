#!/usr/bin/env python3
"""C4 diagnostic: +100 K higher-temperature 100 um liquid droplet, 10 us.

Standalone MFC case derived from:
  runs/shockdroplet_air_sk54/alpha_source_fix/
  C4_long_domain_restartable_base_2p0us/case.py

Purpose:
  Test whether a conservative pre-shock ambient-temperature increase changes
  the SK54 Mach-3 shock/droplet response.  C4 geometry, grid, droplet liquid
  properties, chemistry, molecular diffusion, phase-change setup, numerics,
  output cadence, and restart/output-related controls are otherwise preserved.

Intentional diagnostic change:
  T1 = T1_BASE + T1_OFFSET = 490.290629373 K + 100 K.
  The post-shock state is recomputed from normal-shock relations using MS=3.0
  into stationary upstream gas; post-shock pressure is not an independent input.
  The trace dodecane-vapor gas constant is inferred from the base C4 upstream
  trace-vapor state, so upstream/post-shock alpha_rho(2) follows p/(R_vap*T).
"""

import json

# Top-level controls.
MS = 3.0  # shock Mach into stationary upstream gas
P1 = 101325.0
T1_BASE = 490.2906293729238  # inferred from C4 upstream N2/O2 gas density/composition
T1_OFFSET = 100.0
T1 = T1_BASE + T1_OFFSET
T_STOP = 1.0e-5

# C4/SK54 gas composition used for the pre-shock oxidizer.
Y_N2 = 0.7670907820415769
Y_O2 = 0.2329092179584231
MW_N2 = 0.028014  # kg/mol
MW_O2 = 0.031998  # kg/mol
R_UNIVERSAL = 8.31446261815324  # J/(mol*K)
MW_AIR = 1.0 / (Y_N2 / MW_N2 + Y_O2 / MW_O2)
R_AIR = R_UNIVERSAL / MW_AIR

# MFC's stored fluid_pp(3)%gamma is the stiffened-gas Gamma parameter;
# for ideal-gas air gamma_phys = 1 + 1/Gamma.
FLUID3_GAMMA_PARAM = 2.5000000000000004
GAMMA_AIR = 1.0 + 1.0 / FLUID3_GAMMA_PARAM

# Infer the trace dodecane vapor gas constant from the base C4 pre-shock state.
C4_ALPHA_VAP_UPSTREAM = 1.5e-05
C4_ALPHA_RHO_VAP_UPSTREAM = 6.502361578150291e-05
RHO_VAP_C4_INTRINSIC = C4_ALPHA_RHO_VAP_UPSTREAM / C4_ALPHA_VAP_UPSTREAM
R_VAP = P1 / (RHO_VAP_C4_INTRINSIC * T1_BASE)


def normal_shock_state(M, gamma, p1, t1, r_air, r_vap):
    """Return normal-shock downstream state for stationary upstream gas."""
    p2_p1 = 1.0 + 2.0 * gamma / (gamma + 1.0) * (M * M - 1.0)
    rho2_rho1 = ((gamma + 1.0) * M * M) / ((gamma - 1.0) * M * M + 2.0)
    ...
    shock_speed = M * (gamma * r_air * t1) ** 0.5
...
case = {
    ...
    'patch_icpp(1)%alpha_rho(2)': 1.5e-05 * rho_vap_1,
    'patch_icpp(1)%alpha_rho(3)': 0.99997 * rho_air_1,
    ...
    'patch_icpp(2)%alpha_rho(2)': 1.5e-05 * rho_vap_2,
    'patch_icpp(2)%alpha_rho(3)': 0.99997 * rho_air_2,
    ...
    'patch_icpp(3)%alpha_rho(2)': 1.5e-05 * rho_vap_1,
    'patch_icpp(3)%alpha_rho(3)': 1.5e-05 * rho_air_1,
